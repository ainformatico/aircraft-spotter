defmodule Decoder.ICAO do
  @moduledoc """
  Returns information based on an ICAO hexadecimal number.

  Ported from:
  https://github.com/flightaware/dump1090/blob/cdba7566fde1ab45186f2dc4ea53e0594de484b1/public_html/flags.js
  """
  @icao_ranges [
    ...
    %{
      ...
      :flag_image => "Soloman_Islands.png"
    },
  ...
  @doc """
  Decodes the ICAO number and returns the known information for the aircraft.

  ## Examples

      iex> Decoder.ICAO.decode("0CA000")
      %{
        :country => "Antigua and Barbuda",
        :flag_image => "Antigua_and_Barbuda.png"
      }

      iex> Decoder.ICAO.decode("000000")
      %{
        :country => "Unassigned",
        :flag_image => nil
      }

      iex> Decoder.ICAO.decode("")
      %{
        :country => "Unassigned",
        :flag_image => nil
      }

      iex> Decoder.ICAO.decode(nil)
      %{
        :country => "Unassigned",
        :flag_image => nil
      }
  """
  @spec decode(nil) :: map()
  def decode(nil), do: unassigned_range()

  @spec decode(String.t()) :: map()
  def decode(""), do: unassigned_range()
  ...
  @doc """
  Returns the :flag_image for a given country.

  ## Examples

      iex> Decoder.ICAO.flag_from_country("Antigua and Barbuda")
      "Antigua_and_Barbuda.png"

      iex> Decoder.ICAO.flag_from_country("localhost")
      nil

      iex> Decoder.ICAO.flag_from_country("")
      nil

      iex> Decoder.ICAO.flag_from_country(nil)
      nil
  """
  @spec flag_from_country(String.t()) :: String.t() | nil
  def flag_from_country(country) do
    (Enum.find(@icao_ranges, &find_country(&1, country)) || unassigned_range())
    |> Map.get(:flag_image)
  end

  defp calculate_icao(icao) do
    hexa = parse_icao(icao)

    (Enum.find(@icao_ranges, &find_in_range(&1, hexa)) || unassigned_range())
    |> Map.drop([:start, :end])
  end

  defp parse_icao(icao) do
    {hexa, _} = Integer.parse(icao, 16)

    hexa
  end

  defp find_country(range, country) do
    range[:country] == country
  end

  defp find_in_range(range, value) do
    value >= range[:start] and value <= range[:end]
  end

  defp unassigned_range do
    %{
      :country => "Unassigned",
      :flag_image => nil
    }
